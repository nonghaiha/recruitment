                            ...
                            <div class="item-job full">
                                <div class="cell name-job">
                                    @if(sizeof($value->find_branch_location) > 0)
                                        <h3 class="title"><a
                                                href="{{route('client.job.detail',['id' => $value['id']])}}">[{{$value->find_branch_location[0]->city}}
                                                ] {{ $value['title'] }} </a></h3>
                                    @endif
                                </div>
                                <div class="cell desc">
                                    ...
                                    @if(sizeof($value->find_branch_location) > 0)
                                        <p class="info"><span class="icon-location6 -ap"></span> Địa điểm phỏng vấn: <a
                                                class="_blank"
                                                href="#">{{ $value->find_branch_location[0]->street . ', ' . $value->find_branch_location[0]->city . ', ' . $value->find_branch_location[0]->country }}</a>
                                        </p>
                                    @endif
                                </div>
                                <div class="cta cell">
                                    <a href="{{route('client.job.detail',['id' => $value['id']])}}" class="btn-apply">Ứng
                                        tuyển </a>
                                </div>
                            </div>
                        @endforeach
